...
BPlusTree.prototype.prev=function(fs=0){
	...
	var i, hist, code, val, len=this.history.length-1;
	if(len>0){
		this.root=undefined;
		var oldHist=[];
		this.remOps.unshift(this.history[len]);
		for(i=0; i<len; i++) oldHist.push(this.history[i]);
		this.history=[];
		for(i=0; i<len; i++){
			hist=oldHist[i];
			code=hist.charAt(0); // a fuer add oder r fuer remove
			val=hist.substr(2); // der Zahlenwert
			if(code=='a') this.add(val);
			else if(code=='r') this.remove(val);
		}
		if(fs==0) this.draw(undefined, -1, -1);
	}
}

BPlusTree.prototype.next=function(ls=0){

	var hist, code, val, len=this.remOps.length;
	if(len>0){
		hist=this.remOps[0];
		code=hist.charAt(0); // a fuer add oder r fuer remove
		val=hist.substr(2); // der Zahlenwert
...
BPlusTree.prototype.firstState=function(){
	...
	if(this.history.length>this.limit){
		while(this.history.length>this.limit) this.prev(1);
		tree.draw(undefined, -1, -1);
	}else if(this.history.length<this.limit){
		while(this.history.length<this.limit) this.next(1);
		tree.draw(undefined, -1, -1);
	}
}

BPlusTree.prototype.lastState=function(){
	if(this.remOps.length>0){
		while(this.remOps.length>0) this.next(1);
		tree.draw(undefined, -1, -1);
	}
}
...
BPlusTree.prototype.add=function(mode) { // Funktion hinzufuegen
	...
	var val=0, index=0, i=0, height=0, rollTime=(this.speed+1)*1000, cc=1, origVal, origLen;
	var nVal=true;
	...
	}else if(mo==-1||mo==-3){ // -1 bedeutet: zufaelligen Wert eingeben. -3 bedeutet: Zufallswert, Baum nicht zeichnen
		val=Math.floor((Math.random() * 999) + 1); // Zufallszahl generieren
	}else{
		val=mo; // wenn der Wert mit dem Parameter uebermittelt wurde
	}
	...
	if(this.root==undefined){ // wenn noch keine Wurzel existiert
		...
		if(mo==-1||mo==-2) this.draw(actNode, val, 1); // Baum im alten Zustand zeichnen (ohne den neuen Wert im Blatt)
...
BPlusTree.prototype.randomTree=function(){ // zufaelligen Baum erstellen
	...
	this.opCount=0;
	var i;
	for(i=0; i<this.limit; i++) this.add(-3);
	tree.draw(undefined, -1, -1);
}
...
BPlusTree.prototype.remove=function(mode){
	...
	if(this.order<0||this.root==undefined){ 
		window.alert("Create the tree first!");
		this.runningOp=false;
		return;	
	}else{
		...
		var parentNode;
		var val=0, index=0, indexVal=0, i=0, j=0, borVal, height=0, rollTime= (this.speed + 1) *1000;
...
BPlusTree.prototype.searchTree=function(mode){
	...
	if(this.order<0||this.root==undefined){
		...
		var mo=parseInt(mode);
		var val=0, index=0, i=0, j=0, height=0, rollTime= (this.speed + 1) *1000;
		var found=false;
